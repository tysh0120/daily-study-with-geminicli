...
describe('RateLimitter', () => {
    ...
    test('runはタスク開始から規定時間を超えたらまだ終了していなくても最大実行数のタスクを実行する', async () => {
        rateLimitter.run(taskMaker.makeTask)
    });

    test('aa', async () => {
        const task = taskMaker.makeTask('task1', 3000);
        await task();
        const execRecords = taskMaker.getExecRecords();
        expect(execRecords.length).toBe(1);
        console.log(execRecords)
        expect(execRecords[0]!.start.getFullYear()).toBe(2026)
    });
});
